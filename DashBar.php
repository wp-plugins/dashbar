    ...
    function admin_pages() {
  	...
  	function display_bar() {
  	  ...
  	  global $current_user;
  		echo '<div id="DashBar">'
  				.'<ul>';
  	  echo '<li><a href="'.get_option('siteurl').'/wp-admin/profile.php">'.__('My Account', $this->domain).'</a><ul><li><a href="'.get_option('siteurl').'/wp-login.php?action=logout">'.__('Logout',$this->domain).'</a></li></ul></li>'
  	  		.'<li><a href="'.get_option('siteurl').'/wp-admin/">'.__('Dashboard',$this->domain).'</a>';
    	if(current_user_can('activate_plugins') || current_user_can('manage_categories') || current_user_can('manage_links') || current_user_can('manage_files') || current_user_can('manage_options')) {
    	  echo '<ul>';
    	  if(current_user_can('manage_categories')) {
    			echo '<li><a href="'.get_option('siteurl').'/wp-admin/categories.php">'.__('Categories',$this->domain).'</a></li>';
    	  }
    	  if (current_user_can('manage_links')) {
    			echo '<li><a href="'.get_option('siteurl').'/wp-admin/link-manager.php">'.__('Links',$this->domain).'</a></li>';
    	  }
    	  if (current_user_can('manage_files')) {
    			echo '<li><a href="'.get_option('siteurl').'/wp-admin/templates.php">'.__('Files',$this->domain).'</a></li>';
    	  }
    	  if (current_user_can('manage_options')) {
    			echo '<li><a href="'.get_option('siteurl').'/wp-admin/options-general.php">'.__('Options',$this->domain).'</a></li>';
    	  }
    	  if (current_user_can('activate_plugins')) {
    			echo '<li><a href="'.get_option('siteurl').'/wp-admin/plugins.php">'.__('Plugins',$this->domain).'</a></li>';
    	  }
    	  echo '</ul>';
    	}
  	  echo '</li>';
  	  if (current_user_can('edit_posts')) {
  		  echo '<li><a href="'.get_option('siteurl').'/wp-admin/post.php">'.__('Write Post',$this->domain).'</a></li>';
    		rewind_posts();
    		if(is_single()||is_page()) {
    			the_post();
    			global $id, $authordata;
      		if(($current_user->ID == $authordata->ID) or current_user_can('edit_others_posts')) {
    	  		echo '<li><a href="'.get_option('siteurl').'/wp-admin/post.php?action=edit&amp;post='.$id.'" title="'.get_the_title().'">'.__('Edit',$this->domain).'</a></li>';
    	  	}
    		} elseif(!is_404()) {
    			if(have_posts()){
    				echo '<li><a href="'.get_option('siteurl').'/wp-admin/edit.php">'.__('Edit',$this->domain).'</a><ul>';
    				while(have_posts()) {
    					the_post();
    					global $id, $authordata;
    					if(($current_user->id == $authordata->ID) or current_user_can('edit_others_posts')) {
    					  echo '<li><a href="'.get_option('siteurl').'/wp-admin/post.php?action=edit&amp;post='.$id.'" title="'.__('Edit :',$this->domain).get_the_title().'">'.get_the_title('','',false).'</a></li>';
    					}
    				}
    				echo '</ul></li>';
    			}
...
<style type="text/css">
#DashBar {position: fixed; top: 0; left: 0;opacity:<?php echo ($this->values['opacity']/100)?>;background: <?php echo $this->values['bgcolor']; ?>;color:<?php echo $this->values['acolor']; ?>;width: 100%;font-family: sans-serif;font-size: <?php echo $this->values['height']; ?>!important;padding:0; margin:0;}
